...
class MainMenu:
    ...
    def click_handler(self, mouse):
        #game starts upon click
        if self.mode == 0:
            if mouse.x >= 450 and mouse.x <= 793 and mouse.y >= 620 and mouse.y <= 775:
                self.start_game()
            if mouse.x >= 450 and mouse.x <= 793 and mouse.y >= 430 and mouse.y <= 587:
                self.mode = 2
            if mouse.x >= 450 and mouse.x <= 793 and mouse.y >= 235 and mouse.y <= 390:
                self.mode = 1
        else:
            self.mode = 0
...
class GameOverScreen:
    def __init__(self):
        ...
        self.backgrnd = []
        for i in range(2):
            self.backgrnd.append(loadImage(PATH+"//images//game_over{0}.png".format(i)))
    ...
    def key_handler(self, key_pressed):
        ...
        if key_pressed == 32:
            ...
            self.is_typing_active = False
        elif self.is_typing_active and key_pressed == 8:
            self.user_name = self.user_name[:-1]
        elif self.is_typing_active and chr(key_pressed).isalpha():
            self.user_name+= chr(key_pressed)
        
    def display(self):
        image(self.backgrnd[int(game.won)], 0, 0)
        ...
        textSize(30)
        textAlign(LEFT)
        text(self.user_name, 630, 33, 200, 40)
        text(str(game.score), 656, 687, 200, 40)
        
    def click_handler(self, mouse):
        # activates username typing
        if mouse.y >= 30 and mouse.y <= 65 and mouse.x >= 620 and mouse.x <= 900:
